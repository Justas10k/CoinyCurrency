...
function LiveRates() {
  ...
  // State variables for currency information
  ...
  const handleDelete = (currency) => {
    ...
    setCurrencies((prevCurrencies) =>
      prevCurrencies.filter((curr) => curr !== currency),
    );
  };
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        const filteredOptions = options.filter(
          (item) =>
            !clickedCurrency.includes(item) && !newCurrencies.includes(item),
        );
  ...
  //fetch exchange rates when selectedCurrency or currencies change
  useEffect(() => {
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        const filteredOptions = options.filter(
          (item) =>
            !selectedCurrency.includes(item) && !currencies.includes(item),
        );
        setCurrencyOptions(filteredOptions);
      })
      .catch((error) => console.error("Error fetching exchange rates:", error));
        ...
        <div className="con">
          ...
          <div className="market-table">
            <div className="table-row">
              ...
              <div className="full-width-end right-side table-padding-left">
                <button
                  ...
                  onClick={showDeleteButton ? handleDone : setShowDeleteButton}
                >
            ...
            <div className="table-row select-table">
              <p className="full-width select-currency table-padding-right">
                {selectedCurrency}
              </p>
              <p className="full-width">{inverse ? "inverse" : "1"}</p>
              <p className="full-width">
                {currenciesInfo[selectedCurrency]?.symbol || "Loading..."}
              </p>
              <p className="full-width">
                {currenciesInfo[selectedCurrency]?.name || "Loading..."}
              </p>
              ...
              (currency) =>
                !rowsToDelete.includes(currency) && (
                  <div className="table-row table-border" key={currency}>
                    <p
                      className={`full-width select-currency table-padding-right ${
                    ...
                    <p className="full-width">
                      {exchangeRates && exchangeRates[currency] !== undefined
                        ? inverse
                          ? (1 / exchangeRates[currency]).toFixed(6)
                          : exchangeRates[currency].toFixed(6)
                        : "Loading..."}
                    </p>
                    <p className="full-width">
                      {currenciesInfo[currency]?.symbol || "Loading..."}{" "}
                    </p>
                    <p className="full-width">
                      {currenciesInfo[currency]?.name || "Loading..."}
                    </p>
                    <div className="full-width-end right-side table-padding-left">
                      <button
                        className={`max-width  ${
                          showDeleteButton ? "delete-button" : "hidden-button"
                        }`}
